...
export default function CondimentosPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (imageFile) {
      const result = await uploadImage(imageFile, 'toppings', 'toppings')
      if (result.url) {
        imageUrl = result.url
      }
    }
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Tem certeza que deseja excluir este condimento?')) {
      const { error } = await supabase
        .from('toppings')
        .delete()
        .eq('id', id)
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="lg:ml-56">
        ...
        <h1 className="text-3xl font-bold text-foreground">Gerenciar Condimentos</h1>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="bg-card border-border text-foreground max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingTopping ? 'Editar Condimento' : 'Novo Condimento'}</DialogTitle>
            </DialogHeader>
                ...
                <div>
                  ...
                  <Select value={formData.category} onValueChange={(value) => setFormData({ ...formData, category: value })}>
                    ...
                    <SelectContent className="bg-muted border-border">
                      ...
                      <SelectItem value="crocante">Crocante</SelectItem>
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div className="w-16 h-16 bg-muted rounded-lg flex items-center justify-center">
                {topping.image_url ? (
                  <img src={topping.image_url} alt={topping.name} className="w-full h-full object-cover rounded-lg" />
                ) : (
                  <ImageIcon className="w-8 h-8 text-foreground/50" />
                )}
              </div>
              <div className="flex gap-2">
                <Button size="sm" variant="ghost" onClick={() => handleEdit(topping)} className="text-foreground hover:bg-muted">
                  <Edit className="w-4 h-4" />
                </Button>
                <Button size="sm" variant="ghost" onClick={() => handleDelete(topping.id)} className="text-red-500 hover:bg-red-500/10">
            ...
            <div className="mt-2 flex items-center gap-2">
              <span className={`px-2 py-1 rounded text-xs ${topping.is_active ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                {topping.is_active ? 'Ativo' : 'Inativo'}
              </span>
              {topping.category && (
